        ...
        <div class="col-xl-12 dashboard-column">
            <header class="box-typical-header panel-heading" style="margin-bottom: 30px;">
                <div class="row">
                    <div class="col-xl-10 col-sm-10 col-xs-9">
                        <a href="{{route('invoice.return.index')}}"><button class="btn btn-primary">Lihat Daftar Faktur Retur</button></a>
                    </div>
            ...
            <section class="card" id="print-area">
                <header class="card-header card-header-lg">
                    Faktur Retur - {{$invoice->payment_status}} - {{$invoice->orderCustomerReturnInvoices[0]->orderCustomerReturn->status}}
                </header>
                        ...
                        <div class="col-lg-12">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    $i = 1;
                                    @endphp
                                    @foreach($invoice->orderCustomerReturnInvoices as $row)
                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>
                                                {{$row->price->name}}
                                            ...
                                            <td>
                                                {{$row->orderCustomerReturn->description}}
                                            </td>
                                            <td>
                                                {{$row->quantity}}
                                            </td>
